...
if __name__ == "__main__":
    # Define
    predictor = Predictor()
    data_paths = sorted(glob.glob('/data/visa/test/*/*/*'))
    weight_path = ['weight_path']
    visa_prompt = json.load(open('jsons/eval/visa/prompt_visa_single.json'))
    for idx in range(len(weight_path)):
        ...
        for weight in weights:
            ...
            os.makedirs(os.path.join('result/visa/mono', timestamp),  exist_ok=True)
            # Setup
            with open(os.path.join('result/visa/mono', timestamp, 'output_full_text.csv'), 'w') as f: pass
            ...
            for data_path in data_paths:
                input_text = visa_prompt[data_path]
                for i, text in enumerate(predictor.predict(image=[data_path], prompt=input_text, temperature=1e-5)):
                    ...
                    if tmp_mode != data_path.split('/')[-2]:
                        with open(os.path.join('result/visa/mono', timestamp, 'output_full_text.csv'), 'a') as f:
                            writer = csv.writer(f)
                    ...
                    if (text.split('ASSISTANT: ')[-1] == 'None' or text.split('ASSISTANT: ')[-1] == 'None.') and data_path.split('/')[-2] == 'Normal':
                        correct += 1
                        total += 1
                        true_or_false = True
                    elif (text.split('ASSISTANT: ')[-1] != 'None' and text.split('ASSISTANT: ')[-1] != 'None.') and data_path.split('/')[-2] != 'Normal':
                        correct += 1
                        total += 1
                        true_or_false = True
                    else:
