...
def calculate_score_by_category(correct_data, generated_data,args,current_date):
    category_name_list = []
    correct_category_data = {}
    generated_category_data = {}
    output_data = {}
    for i in range(len(correct_data)):
        category = correct_data[i]["id"].split('/')[-3]
        if category not in category_name_list:
            category_name_list.append(category)
            correct_category_data[category] = []
            generated_category_data[category] = []
            output_data[category] = None

        correct_category_data[category].append(correct_data[i])
        generated_category_data[category].append(generated_data[i])
        
    for category in category_name_list:
        print(f"Processing category: {category}")
        output_data[category] = calculate_score(correct_category_data[category], generated_category_data[category],args,current_date)
...
def main(args):
    ...
    if args.output_path is None:
        ...
        output_path = os.path.join(generated_json_folder,f"{base_name.split('.')[0]}-category.json")
    else:
    ...
    output_data = calculate_score_by_category(correct_data, generated_data,args,current_date)
