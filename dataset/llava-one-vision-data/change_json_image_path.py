...
# 画像とJSONデータの保存先を指定
DATA_DIR = '/data_ssd/USER_DATA/omote/iam2/LLaVA-OneVision-Data'
PREFIX_DIR = '/data_ssd/USER_DATA/omote/iam2'

def change_image_path(dataset_name):
    """
    Change the image paths in the JSON files of a dataset to point to the correct directory.

    Parameters:
        dataset_name (str): The name of the dataset.
    """
    json_file_path = os.path.join(DATA_DIR,dataset_name, f'{dataset_name}.json')
    ...
    for index,item in enumerate(tqdm(data, desc=f"Processing {dataset_name}")):
        if 'image_path' in item:
            ...
            image_path = image_path[len(PREFIX_DIR)+1:]  # Remove the base directory part
    ...
    # 変更後のデータを保存
    save_json_file_path = os.path.join(DATA_DIR, dataset_name, f'{dataset_name}_relative_path.json')
...
def main():
    ...
    for dataset_name in DATASET_NAMES:
        ...
        change_image_path(dataset_name)
